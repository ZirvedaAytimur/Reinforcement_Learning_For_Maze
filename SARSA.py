...
while run:
    ...
    new_action = select_an_action(new_state)

    if new_state not in obstacles:
        predict = Q[current_state, action]
        target = reward[current_position[0], current_position[1]] + gamma * Q[new_state, new_action]
        Q[current_state, action] += alpha * (target - predict)
